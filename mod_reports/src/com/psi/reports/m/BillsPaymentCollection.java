...
public class BillsPaymentCollection extends ModelCollection {
	protected String id;
	...
	public boolean hasRows() {
		...
		if(acct.getString("USERSLEVEL").equals("MANAGER")){
			DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSBILLSPAYMENT WHERE ACCOUNTNUMBER=? AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",acctno,this.datefrom,this.dateto);
			...
			DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSBILLSPAYMENT WHERE ACCOUNTNUMBER=? AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ? AND UPPER(CASHIER)=? ",acctno,this.datefrom,this.dateto,acct.getString("USERNAME").toUpperCase());
