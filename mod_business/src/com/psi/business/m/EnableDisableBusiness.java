...
public class EnableDisableBusiness
  extends Branch {
  public boolean update() {
    String rr = "";
    DataRowCollection row = SystemInfo.getDb().QueryDataRows("SELECT ACCOUNTNUMBER FROM TBLBRANCHES WHERE STATUS=1 AND KEYACCOUNT = ?", new Object[] { this.accountnumber });
    if (!row.isEmpty()) {
      for (DataRow r : row) {
        rr = String.valueOf(rr) + "'" + r.getString("ACCOUNTNUMBER") + "',";
      }
    } else {
      rr = "'',";
    } 
    ...
    String rrr = "";
    DataRowCollection roww = SystemInfo.getDb().QueryDataRows("SELECT ACCOUNTNUMBER FROM TBLBRANCHES WHERE STATUS=0 AND KEYACCOUNT = ?", new Object[] { this.accountnumber });
    if (!roww.isEmpty()) {
      for (DataRow r : roww) {
        rrr = String.valueOf(rrr) + "'" + r.getString("ACCOUNTNUMBER") + "',";
      }
    } else {
      rrr = "'',";
    } 
    if (this.status.equals("DISABLE")) {
      ...
      return (SystemInfo.getDb().QueryUpdate(query.toString(), new Object[] { this.accountnumber, this.accountnumber, this.accountnumber }) > 0);
    } 
  ...
  public boolean exist() { return (SystemInfo.getDb().QueryDataRow("SELECT * FROM TBLBUSINESS WHERE ACCOUNTNUMBER=?", new Object[] { this.accountnumber }).size() > 0); }
}
