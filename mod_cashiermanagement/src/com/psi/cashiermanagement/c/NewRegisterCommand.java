...
import com.psi.cashiermanagement.m.NewCashierRegistration;
import com.psi.cashiermanagement.util.AuditTrail;
import com.psi.cashiermanagement.util.EmailUtils;
	...
	public IView execute() {	
		...
		try {
			...
			if(sess.exists()) {
				...
				NewCashierRegistration reg = new NewCashierRegistration();
				...
				if(!reg.register()){
					reg.setState(new ObjectState("99",reg.getState().getMessage()));
					AuditTrail audit  = new AuditTrail();
		    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(username);
