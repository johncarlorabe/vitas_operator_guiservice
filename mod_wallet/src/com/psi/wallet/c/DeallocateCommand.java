...
public class DeallocateCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				Deallocate topup = new Deallocate();
				...
				topup.setAuthorizedSession(sess);
						...
						try {
							if(topup.topup()){
								topup.setState(new ObjectState("00", "Deallocated Successfully"));
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid(accountnumber);
