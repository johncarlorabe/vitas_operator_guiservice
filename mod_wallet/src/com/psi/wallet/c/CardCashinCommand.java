...
public class CardCashinCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				CardCashin topup = new CardCashin();
				...
				topup.setDescription("CARD CASH IN");
				...
				topup.setAuthorizedSession(sess);
						...
						if(!topup.exist()){
							topup.setState(new ObjectState("02", "Card does not exist"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(cardnumber);
						...
						if(topup.isLocked()){
							topup.setState(new ObjectState("03", "Card is locked"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(cardnumber);
						...
						if(!topup.updateStatus()){
							topup.setState(new ObjectState(topup.getState().getCode(),topup.getState().getMessage()));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(cardnumber);
