...
public class EditCardLockStatusCommand extends UICommand {
	...
	public IView execute() {
		...
		try {
			...
			String statuslog = this.params.get("IsLocked").equals("YES")?"(LOCK)":"(UNLOCKED)";
			...
			if (sess.exists()) {
				...
				if (regacct.exists()) {
					if (passwordCheck.exists()) {	
						...
						regacct.setStatus(this.params.get("IsLocked"));
						
							if (regacct.updateLockStatus(reason)) {
								regacct.setState(new ObjectState("00" ,"Successfully Updated Card Lock Status"));
								...
								regacct.setState(new ObjectState("PSI-7300-01","Unsuccessful Updating Card Lock Status"));
								AuditTrail audit = new AuditTrail();
					    		audit.setIp(regacct.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid(msisdn);
