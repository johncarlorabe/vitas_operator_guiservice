...
public class SearchCardInfoCommand extends UICommand{
	...
	public IView execute() {
	    ...
	    try
	    {
	      ...
	      if(model.validate()){
    		  	ObjectState state = new ObjectState("PSI-7100-02", "Please input a valid customer mobile number");
    		  
				AuditTrail audit  = new AuditTrail();
	    		audit.setIp(model.getAuthorizedSession().getIpAddress());
	    		audit.setModuleid(String.valueOf(this.getId()));
	    		audit.setEntityid(msisdn);
	    		audit.setLog("Please input a valid customer mobile number");
	    	  ...
	    	  if(model.hasRows()){
	    		  ...
	    		  ObjectState state = new ObjectState("PSI-7100-01", "Mobile number doesn't exists");
	    		  AuditTrail audit  = new AuditTrail();
					audit.setIp(model.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(msisdn);
		    		audit.setLog("Mobile number doesn't exists");
		    		audit.setStatus("PSI-7100-01");
		    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
